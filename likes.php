      ...
      if($select_likes->rowCount() > 0){
         while($fetch_likes = $select_likes->fetch(PDO::FETCH_ASSOC)){
            ...
            if($select_contents->rowCount() > 0){
               ...
               $select_publ = $conn->prepare("SELECT * FROM `publisher` WHERE id = ?");
               $select_publ->execute([$fetch_contents['publisher_id']]);
               $fetch_publis = $select_publ->fetch(PDO::FETCH_ASSOC);
   ?>
   <div class="box">
      <div class="publisher">
