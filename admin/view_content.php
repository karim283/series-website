...
<section class="view-content">
   ...
   <?php
      $select_content = $conn->prepare("SELECT * FROM `content` WHERE id = ? AND publisher_id = ?");
      $select_content->execute([$get_id, $publ_id]);
      if($select_content->rowCount() > 0){
         while($fetch_content = $select_content->fetch(PDO::FETCH_ASSOC)){
            $video_id = $fetch_content['id'];

            $count_likes = $conn->prepare("SELECT * FROM `likes` WHERE publisher_id = ? AND content_id = ?");
            $count_likes->execute([$publ_id, $video_id]);
            $total_likes = $count_likes->rowCount();

            $count_comments = $conn->prepare("SELECT * FROM `comments` WHERE publisher_id = ? AND content_id = ?");
            $count_comments->execute([$publ_id, $video_id]);
