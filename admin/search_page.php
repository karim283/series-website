...
if(isset($_POST['delete_playlist'])){
   ...
   $verify_playlist = $conn->prepare("SELECT * FROM `playlist` WHERE id = ? AND publisher_id = ? LIMIT 1");
   $verify_playlist->execute([$delete_id, $publ_id]);
...
<section class="contents">
   ...
   <?php
      ...
      $search = $_POST['search'];
      $select_videos = $conn->prepare("SELECT * FROM `content` WHERE title LIKE '%{$search}%' AND publisher_id = ? ORDER BY date DESC");
      $select_videos->execute([$publ_id]);
...
<section class="playlists">
   ...
   <div class="box-container">
      ...
      if(isset($_POST['search']) or isset($_POST['search_btn'])){
         $search = $_POST['search'];
         $select_playlist = $conn->prepare("SELECT * FROM `playlist` WHERE title LIKE '%{$search}%' AND publisher_id = ? ORDER BY date DESC");
         $select_playlist->execute([$publ_id]);
